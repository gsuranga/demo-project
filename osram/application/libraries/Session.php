...
// Is there a corresponding session in the DB?
        if ($this->sess_use_database === TRUE) {
            if ($this->PDO_conn == TRUE) {
                $stmt = $this->CI->db->prepare("SELECT * FROM $this->sess_table_name WHERE session_id = :session_id");

                if ($this->sess_match_ip == TRUE) {
                    if ($this->sess_match_useragent == TRUE) {
                        $stmt = $this->CI->db->prepare("SELECT * FROM $this->sess_table_name WHERE session_id = :session_id AND ip_address = :session_ip AND user_agent = :user_agent");
                        $stmt->bindParam(':user_agent', $session['user_agent'], PDO::PARAM_STR, 120);
                    } else {
                        $stmt = $this->CI->db->prepare("SELECT * FROM $this->sess_table_name WHERE session_id = :session_id AND ip_address = :session_ip");
                    }
                    $stmt->bindParam(':session_ip', $session['ip_address'], PDO::PARAM_STR, 45);
                } elseif ($this->sess_match_useragent == TRUE) {
...
// Run the update query
        if ($this->PDO_conn == TRUE) {
            ...
            $stmt->bindParam(':last_activity', $this->userdata['last_activity'], PDO::PARAM_INT, 10);
...
// Save the data to the DB if needed
        if ($this->sess_use_database === TRUE) {
            if ($this->PDO_conn == TRUE) {
                ...
                $stmt->bindParam(':session_id', $this->userdata['session_id'], PDO::PARAM_STR, 40);
                $stmt->bindParam(':ip_address', $this->userdata['ip_address'], PDO::PARAM_STR, 45);
                $stmt->bindParam(':user_agent', $this->userdata['user_agent'], PDO::PARAM_STR, 120);
                $stmt->bindParam(':last_activity', $this->userdata['last_activity'], PDO::PARAM_INT, 10);
                $stmt->execute();
            } else {
