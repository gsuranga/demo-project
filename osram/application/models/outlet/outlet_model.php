 ...
 */
class outlet_model extends C_Model {
    ...
    function viewAllOutlet() {
        ...
        if ($this->session->userdata('user_type') == "Sales Rep") {
            $empId = $this->session->userdata('id_employee');

            $append = "and ehp.id_employee = '$empId'";
        }
...
//        $query = "	SELECT 
//		otr.outlet_code as outlet_code,
//		t . *,
//		oc.outlet_category_name as outlet_category_name
//	FROM
//		tbl_outlet t
//			inner join
//		tbl_outlet_category oc
//			inner join
//		tbl_outlet_registration otr
//			inner join
//		tbl_outlet_has_branch tohb ON t.id_outlet = tohb.id_outlet
//			inner join
//		tbl_territory tr ON tohb.id_territory = tr.id_territory
//			inner join
//		tbl_employee_has_place ehp ON ehp.id_territory = tr.id_territory
//	where
//		otr.id_outlet_registration = t.id_outlet_registration
//			AND t.id_outlet_category = oc.id_outlet_category
//			AND t.outlet_status = 1 {$append}";
...
WHERE tbo.`id_outlet_category` LIKE '%$outlet_cat_name%' AND tbo.`outlet_name` LIKE '%$outlet_name%' AND tbo.`outlet_status`='1' LIKE '%$outlet_status%' AND tor.`outlet_code` LIKE '%$outlet_code%' AND tt.`territory_name` LIKE '%$territory_name%' AND tbhd.`id_division` LIKE '%$division_name%' GROUP BY tbo.id_outlet ";
    ...
    function getOutletNameByEmp($q, $select) {


        if ($this->session->userdata('user_type') == "Sales Rep") {
            $empId = $this->session->userdata('id_employee');

            $append = "where tehp.id_employee = '$empId'";
        }


        $sql = "select 
    tt.id_territory, tt.id_parent, tt.territory_name
    from
    tbl_territory tt,
    (select 
        tt.territory_hierarchy, tehp.id_territory
    from
        tbl_employee_has_place tehp
    left join tbl_territory tt ON tehp.id_territory = tt.id_territory
    {$append}) temp
    where
    temp.id_territory = tt.id_parent
		and tt.territory_status = 1
        and tt.territory_hierarchy like CONCAT('%', temp.territory_hierarchy, '%')
        and tt.territory_hierarchy like CONCAT('%', temp.territory_hierarchy, ',', '%') and tt.territory_name LIKE '%$q%'";
        ...
        $sql="select 
    division_name, id_division
from
    tbl_division
where
    division_name like '$q%'
        and division_status = 1";
        $query = $this->db->query($sql);
        $result = $query->result('array');
        $json_array = array();
        foreach ($result as $row) {
