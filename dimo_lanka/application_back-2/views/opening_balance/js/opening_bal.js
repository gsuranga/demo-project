...
function get_pname(id){
    ...
    $("#autoproduct_name_1"+id).autocomplete({
        source: "get_pname",
        minLength: 1,
        select: function(event, ui) {
            console.log(ui);
            $('#product_name_1'+id).val(ui.item.id);
            $('#item_cat_'+id).val(ui.item.ict_name);
            $('#product_code_' + id).val(ui.item.item_code);
            $('#price_' + id).val(ui.item.ip_price);
        //            get_data(1);
        get_category(id);
        }
    });
...
function get_category (id){
    var id_item=$("#product_name_1"+id).val();
    $.ajax({
        type: 'POST',
        url: 'get_category',
        data:{
                    ...
                    <td width='2%'>\n\
                        <table>\n\
                            <tr>\n\
                                ...
                                <td width='25%'><input type='text'   id='autoproduct_name_1" + rowcount + "'  name='autoproduct_name_1" + rowcount + "' onkeyup='get_pname(" + rowcount + ");'  style='width:95%' /></td>\n\
                                <td width='10%'> \n\
                                <input type='hidden' id='item_cat_1" + rowcount + "' name='item_cat_1" + rowcount + "'  style='width:10%' readonly='readonly'/>\n\
                                <input type='text'  id='item_cat_" + rowcount + "' name='item_cat_" + rowcount + "'  style='width:90%' readonly='readonly'/>\n\
                                </td>\n\ \n\
                                <td width='10%'><input type='hidden' id='product_id_" + rowcount + "' name='product_id_" + rowcount + "'  style='width:10%' readonly='readonly'/>\n\
                                <input type='text'  id='product_code_" + rowcount + "' name='product_code_" + rowcount + "'  style='width:90%' readonly='readonly'/>\n\
                                </td>\n\
                                <td width='10%'><input type='text' value='0.00' style='width:90%' id='price_" + rowcount + "' name='price_" + rowcount + "' onkeyup='calculateTotal(" + rowcount + ");' readonly='readonly'/></td>\n\
