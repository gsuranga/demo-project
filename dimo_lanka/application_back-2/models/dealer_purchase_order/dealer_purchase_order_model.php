...
class dealer_purchase_order_model extends C_Model {
    ...
    public function viewAllPendingOrders() {
        $sql = "select  po.purchase_order_id, po.purchase_order_number ,br.branch_name,d.delar_account_no,d.delar_shop_name, po.date, po.time, round(po.amount,2) as amount,d.delar_id  from tbl_dealer_purchase_order as po left join  tbl_dealer as d on po.dealer_id=d.delar_id inner join tbl_branch as br on d.branch_id=br.branch_id where po.complete=0 group by po.purchase_order_id";
        return $this->db->mod_select($sql);
    }

    public function viewAllPurchaseOrderDetails($data_array) {
        $sql = "select po.purchase_order_detail_id,po.purchase_order_id,po.item_id,i.item_part_no,i.description,po.item_qty,po.unit_price,po.status from tbl_dealer_purchase_order_detail as po inner join tbl_item as i on po.item_id=i.item_id where po.purchase_order_id = " . $data_array['token_purchase_order_iD'] . " and po.status='1'";
        return $this->db->mod_select($sql);
    }

    public function viewAllAcceptedOrders($dealer_id) {
        $sql = "select po.purchase_order_id, po.purchase_order_number, br.branch_name, d.delar_account_no, d.delar_shop_name, po.date, po.time, round(po.amount,2) as amount,d.delar_name,d.mobileO,d.mobileP from tbl_dealer_purchase_order as po inner join tbl_dealer as d ON po.dealer_id = d.delar_id inner join tbl_branch as br ON d.branch_id = br.branch_id where po.complete = 1 and d.delar_id = '" . $dealer_id . "' order by po.purchase_order_id";
        return $this->db->mod_select($sql);
    }

    public function getLastAcceptedOrders() {
        $sql = "select po.purchase_order_id, po.purchase_order_number, br.branch_name, d.delar_account_no, d.delar_shop_name, po.date, po.time, round(po.amount, 2) as amount from tbl_dealer_purchase_order as po inner join tbl_dealer as d ON po.dealer_id = d.delar_id inner join tbl_branch as br ON d.branch_id = br.branch_id where po.complete = 1 and po.date between date_sub(curdate(), interval 7 day) and curdate() order by po.purchase_order_id";
        return $this->db->mod_select($sql);
    }
    ...
    public function viewAllAcceptedPurchaseOrderDetails($order_id) {
        $sql = "select po.purchase_order_detail_id,po.purchase_order_id,po.item_id,i.item_part_no,i.description,po.item_qty,accepted_qty,po.unit_price,po.status from tbl_dealer_purchase_order_detail as po inner join tbl_item as i on po.item_id=i.item_id where po.purchase_order_id = " . $order_id . " and po.status=1";
        return $this->db->mod_select($sql);
    }
