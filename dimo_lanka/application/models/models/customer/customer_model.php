...
//============================insert data tbl_vehicle_driver_details======================
        ...
        if (empty($data_Array['owner_id']) && empty($data_Array['ownerConNo_id']) && empty($data_Array['ownerEmail_id'])) {
            ...
            $owner_in_cus = array(
                'cust_vehicle_owner' => $owner_id
            );
            $this->db->update("tbl_customer", $owner_in_cus, $where4);
        } else if (isset($data_Array['owner_id'])) {
            ...
            $oldowner = array(
                'cust_vehicle_owner' => $data_Array['owner_id']
            );

            $this->db->update("tbl_customer", $oldowner, $where);
        }
...
//                've3' => $data_Array['vehicle_3_' . $y],
        ...
        if (isset($data_Array['token_dealer_no_'])) {
            ...
            for ($D = 0; $D <= $rows3; $D++) {
                ...
                $De = array(
                    'vehicle_id' => $vehicle_id,
                    'part_id' => $data_Array['dealer_select_' . $D], // selecting part
                    'type_id' => $data_Array['PurchasePart_type_id_' . $D], //purchase part type
                    'dealer_name' => $data_Array['selected_name_Hid_' . $D], // selected dealer or vehicle type id
                    'garage_name' => $data_Array['garage_name_id_' . $D], //garage name id
                    'status' => 1,
                );
...
where
    ...
    //Dealers_Garages_Purchased_parts   ================== harshan ====================== start ======>>>>>>>>>>>>>>>

    public function Dealers_Garages_Purchased_model($vehi_id) {
        $sql = "select
            dgp.id,           
    dgp.part_id,
    dgp.vehicle_id,
...
g.garage_name,
  g.garage_id
from
     tbl_dealers_garages_purchased_parts as dgp
        inner join
    tbl_vehicle_part_type as vp ON vp.part_type_id = dgp.part_id
 inner join
    tbl_garage as g ON g.garage_id = dgp.garage_name
 inner join
    tbl_dealer as d on d.delar_id = dgp.dealer_name
 
where
    dgp.status = 1 and  dgp.type_id = 1 and dgp.vehicle_id = '$vehi_id' ";
...
//
//where
//    dgp.status = 1 and dgp.vehicle_id = '$vehi_id'";
        return $this->db->mod_select($sql);
    }

    public function get_dealer_new_shop($vehi_id) {
        $sql = "select
            dgp.id,           
    dgp.part_id,
    dgp.vehicle_id,
...
from
 ...
 inner join
    tbl_garage as g ON g.garage_id = dgp.garage_name
 inner join
  tbl_non_reg_shops as nrs on nrs.shop_id = dgp.dealer_name
 
where
    dgp.status = 1 and  dgp.type_id = 2 and dgp.vehicle_id = '$vehi_id' ";
...
vp.part_type_id,
	...
	from
		...
		tbl_vehicle_part_type as vp
	where
		vp.status = 1 ";
        return $this->db->mod_select($sql);
    }

    //Dealers_Garages_Purchased_parts   ================== harshan ====================== end ======>>>>>>>>>>>>>>>
    //
    //
        //Dealers_Garages_Purchased_parts update  ================== harshan ====================== start ======>>>>>>>>>>>>>>>

    public function update_Dealers_Garages_Purchased($data) {
        //print_r($data);
        $rows = $data['token_dealers_garages'];

        for ($x = 0; $x <= $rows; $x++) {

            $where = array(
                'id' => $data['row_' . $x],
                'vehicle_id' => $data['vehi_id']
            );

            print_r($where);
            if (isset($data['part_type_' . $x])) {

                $setdata = array(
                    'part_id' => $data['part_type_' . $x],
                    'type_id' => $data['PurchasePart_type_id_' . $x],
                    'dealer_name' => $data['selected_name_Hid_' . $x],
                    'garage_name' => $data['garage_name_id_' . $x],
                    'status' => 1
                );
                $this->db->update("tbl_dealers_garages_purchased_parts", $setdata, $where);
            }
        }
        $this->db->__endTransaction();
        return $this->db->status();
...
// add new rows tin update method =================== harshan ====================== start ============>>>>>>>>>
    public function update_Dealers_Garages_Purchased_new($data) {
        //print_r($data);
        $rows = $data['token_dealers_garages'];

        for ($x = 0; $x <= $rows; $x++) {

            if (isset($data['dealer_select_' . $x])) {
                $setdata = array(
                    'vehicle_id' => $data['vehi_id'],
                    'part_id' => $data['dealer_select_' . $x],
                    'type_id' => $data['PurchasePart_type_id_new' . $x],
                    'dealer_name' => $data['selected_name_Hid_new' . $x],
                    'garage_name' => $data['garage_name_new_id_' . $x],
                    'status' => 1
                );
                $this->db->insert("tbl_dealers_garages_purchased_parts", $setdata);
            }
        }
        $this->db->__endTransaction();
        return $this->db->status();
...
// add new rows tin update method =================== harshan ====================== end ============>>>>>>>>>
//Dealers_Garages_Purchased_parts update  ================== harshan ====================== end ======>>>>>>>>>>>>>>>
    ...
    public function update_other_vehicles($data) {
        ...
        $rows = $data['token_arres_of_trevel'];
        for ($i = 0; $i <= $rows; $i++) {
            $where = array(
            ...
            if (isset($data['ve' . $i]) && !empty($data['ve' . $i])) {
...
//update other vehicle new ================================== harshan ========start==============>>>>>>>
    public function update_other_vehicles_new($data) {
        //print_r($data);
        ...
        for ($i = 0; $i <= $rows; $i++) {
            ...
            if (isset($data['ve_new' . $i]) && !empty($data['ve_new' . $i])) {
                ...
                print_r($updateVehicle);
                //   $this->db->__beginTransaction();
                $this->db->insert("tbl_customer_other_vehicle", $setdata);
            }
        }
        $this->db->__endTransaction();
        return $this->db->status();
    ...
    //update other vehicle new ================================== harshan ========end==============>>>>>>>
...
// update ownercontacts ==================================>>>>>
    public function update_owner_contacts($data) {
        ...
        if (isset($data['owner'])) {
            ...
            $this->db->insertData("tbl_vehicle_owner", $setdata);
            $owner_ids = $this->db->insert_id();
            ...
            if (isset($data['owner_id'])) {
                ...
                $data1 = array(
                    'cust_vehicle_owner' => $owner_ids
                );
                $this->db->update("tbl_customer", $data1, $where);
            }
        }
...
//        return $this->db->status();
        ...
        for ($x = 0; $x <= $rows; $x++) {
            ...
            if (isset($dataArray['driver_name_' . $x])) {
                $driver = array(
                    ...
                    'driverConNo' => $dataArray['driverConNo_' . $x],
                    'driverEmail' => $dataArray['driverEmail_' . $x],
                    'status' => 1
                );
                //print_r($driver);
        ...
        for ($x = 0; $x <= $rows; $x++) {
            if (isset($dataArray['driver_name_new_' . $x])) {
                $driver_new = array(
                    ...
                    'driverConNo' => $dataArray['driverConNo_new_' . $x],
                    'driverEmail' => $dataArray['driverEmail_new_' . $x],
                    'status' => 1
                );
    ...
    // update garage and dealers delete =======start=============== harshan============>>>>>>>>>>>>>>>>>>>>>

    public function delete_garage_dealers($id) {
        $details = array("status" => '0');
        $where = array(
            "id" => $id
        );
        $this->db->__beginTransaction();
        $this->db->update('tbl_dealers_garages_purchased_parts', $details, $where);
        $this->db->__endTransaction();
        return $this->db->status();
    }

    // update garage and dealers delete =======end================= harshan============>>>>>>>>>>>>>>>>>>>>>
...
///delete vehicle================== harshan ============>>>>
    public function delete_vehicle($id) {
        ...
        $this->db->__beginTransaction();
        $this->db->update('tbl_customer_other_vehicle', $details, $where);
        $this->db->__endTransaction();
        return $this->db->status();
    }
