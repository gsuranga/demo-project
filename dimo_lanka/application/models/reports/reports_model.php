...
class reports_model extends C_Model {
    ...
    public function dealer_profile(){
         ...
         $sql = "SELECT * FROM tbl_dealer WHERE status= '1'";
         return $this->db->mod_select($sql);
        
    }
    
    public function paymentreport1($acc,$status,$aso,$stdate,$enddate){
        $cont='';
        $sta='';
        $fromaso='';
        $date_fix='';
       if($acc){
        $sql = "SELECT delar_id from tbl_dealer WHERE delar_account_no = '$acc' ";
        $result = $this->db->mod_select($sql);
        $delar_id =   $result[0]->delar_id;
        
        $cont = "tdo.dealer_id = '".$delar_id."' and ";
       }if($status == '1'){
        $sta = "glv.status = '0'  and glv.inprogress = '0' and ";   
       }
       if($status == '2'){
          $sta = " glv.inprogress = '4' and "; 
       }
       if($status == '3'){
          $sta = " glv.inprogress = '0' and glv.status = '1' and ";  
       }
       if($status == '4'){
          $sta = " glv.inprogress = '0' and glv.status = '3' and ";  
       }if($aso){
          $fromaso = "glv.sales_officer_id = '".$aso."' and ";
       }
        $sql="SELECT tdo.invoice_no,tdo.wip_no,glv.voucher_no,glv.cash_payment,td.delar_account_no,td.delar_name,glv.appr_date,
              glv.added_date,tdo.due_date,tdo.tgt_collection_dt,glv.aprover,glv.status,glv.inprogress
              FROM tbl_dealer_deliver_order tdo
              left OUTER JOIN tbl_garage_loyality_vouture glv on glv.deliver_order_id = tdo.deliver_order_id
              inner join tbl_dealer td on td.delar_id = tdo.dealer_id
              where {$cont}{$sta}{$fromaso}  glv.voucher_no != ''";
  ...
  //----------------------------------------------------------------------  
      public function paymentcheck($acc,$status,$aso,$stdate,$enddate){
          $cont='';
          $sta='';
          $fromaso='';
          if($acc) {
        ...
        $cont = "tdo.dealer_id = '".$delar_id."' and"; 
              ...
              $sta="tcp.realized_status = '0' and tcp.inprogress = '0' and  ";
          
          }if($status == '2'){
              $sta="tcp.inprogress = '5' and  ";
          }if($status == '3'){
              $sta="tcp.inprogress = '0' and tcp.realized_status = '1' and   ";
          
          }if($status == '4'){
              $sta="tcp.inprogress = '0' and tcp.realized_status = '3' and   ";
          }if($aso){
             $fromaso = "tcp.sales_officer_id = '".$aso."' and ";
             }
          
       $sql="SELECT tdo.invoice_no,tdo.wip_no,tcp.cheque_no,tcp.realized_status,tcp.cheque_payment,tcp.appr_date,td.delar_account_no,td.delar_name,
              tcp.added_date,tdo.due_date,tdo.tgt_collection_dt,tcp.aprover,tcp.status,tcp.inprogress
              FROM tbl_dealer_deliver_order tdo
              left OUTER JOIN tbl_cheque_payment tcp on tcp.deliver_order_id = tdo.deliver_order_id
              inner join tbl_dealer td on td.delar_id = tdo.dealer_id
              where {$cont}{$sta}{$fromaso} tcp.cheque_no != '' and tcp.status ='0' ";
    ...
    public function paymentbankdep($acc,$status,$aso,$stdate,$enddate){
        $cont='';
        $sta='';
        $fromaso='';
       if($acc){
        ...
        $delar_id =   $result[0]->delar_id;
          ...
          $sta = "tbp.status = '5' and  tbp.progress = '0' and "; 
       }if($status == '2'){
          $sta = "tbp.progress = '2' and "; 
       }if($status == '3'){
          $sta = "tbp.progress = '0' and tbp.status = '0' and "; 
       
       }if($status == '4'){
          $sta = "tbp.progress = '0' and tbp.status = '4' and "; 
       
       }if($aso){
        $fromaso = "tbp.sales_officer_id = '".$aso."' and ";
        }
        $sql="SELECT tdo.invoice_no,
              tdo.wip_no,
              tbp.deposit_payment,
              tbp.account_no,
              tbp.deposit_slip_image,
              tbp.slip_no,
              tbp.deposit_date,
              tbp.added_date,
              tbp.progress,
              tdo.tgt_collection_dt,
              tdo.due_date,
              tbp.appr_date,
              tbp.status,
              tbp.aprover,
              td.delar_account_no,td.delar_name
              FROM tbl_dealer_deliver_order tdo
              left OUTER JOIN  tbl_tab_bank_deposit_payment tbp on tbp.deliver_order_id = tdo.deliver_order_id
              inner join tbl_dealer td on td.delar_id = tdo.dealer_id
              where {$cont}{$sta}{$fromaso} tbp.account_no != ''";
    ...
    //paymentreportreturn
    public function paymentreportreturn($acc,$status,$aso,$stdate,$enddate){
        $cont='';
        $sta='';
        $fromaso='';
        $date_range='';
     ...
     $delar_id =   $result[0]->delar_id;
        ...
        $sta = "tab.status = '0' and tab.inprogrss = '0' and ";
        }if($status== '2'){
        $sta = "tab.inprogrss = '1' and ";
        }if($status== '3'){
        $sta = "tab.inprogrss = '0' and tab.status = '1' and ";
        }if($status== '4'){
        $sta = "tab.inprogrss = '0' and tab.status = '4' and ";
        }if($aso){
         $fromaso = "tab.sales_officer_id = '".$aso."' and ";
        }
      $sql="SELECT tdo.invoice_no,tdo.wip_no,tdo.due_date,tab.add_date,tdo.tgt_collection_dt,tab.inprogrss,tab.aprover,tab.part_no,tab.qty,tab.status,tab.return_id,tab.appr_date,td.delar_account_no,td.delar_name
      FROM tbl_dealer_deliver_order tdo left outer JOIN 
      tbl_return_tab tab on tab.deliver_order_id = tdo.deliver_order_id
      inner join tbl_dealer td on td.delar_id = tdo.dealer_id
      where  {$sta} {$cont} {$fromaso}  tab.return_id !='' ";
...
//-----------------------------------------------------------------------------------
    ...
    public function get_branch() {
        $sql = "select branch_id,branch_name from tbl_branch where status = 1 ";
        return $this->db->mod_select($sql);
    }
    
    
//     public function get_aso() {
//        $sql = "select sales_officer_id,sales_officer_name from tbl_sales_officer where status = 1 ";
//        return $this->db->mod_select($sql);
//    }
    public function get_aso($branch_id){
        
        $sql="SELECT
                  tbl_branch.branch_id,tbl_branch.branch_name, tbl_sales_officer.sales_officer_id,tbl_sales_officer.branch_id,tbl_sales_officer.sales_officer_name	
              FROM 
                tbl_sales_officer,tbl_branch
              Where 
                tbl_sales_officer.branch_id=tbl_branch.branch_id
                and tbl_sales_officer.status=1 
                and tbl_sales_officer.branch_id= $branch_id";
     return $this->db->mod_select($sql);
    }
...
";
    ...
    //----------------new-for Competitor Report------------------------------------------------------------------------

    public function getAllcomParts($q, $select) {
...
//
//FROM tbl_item ti 
//inner join tbl_all_sales als on als.part_no = ti.item_part_no
//
//inner JOIN tbl_dealer td on td.delar_account_no = als.acc_no
//
//INNER join tbl_sales_officer tso on tso.sales_officer_id = td.sales_officer_id
//
//
//
//where ti.status = '1' and  ti.item_part_no like '$q%' group by ti.item_part_no";
# $sql = "SELECT item_part_no FROM tbl_item where status = '1' and item_part_no like '$q%' ";
        #echo $sql= "select * from tbl_item where status='1' and item_part_no like '$q%'";
        $query = $this->db->query($sql);
        $result = $query->result('array');
        $json_array = array();
        foreach ($result as $row) {
    ...
    public function getSalesOfficer($q, $select) {
        ...
        $sql = "select sales_officer_name
from tbl_sales_officer
where sales_officer_name like '$q%'  ";
    ...
    //getDealerAcc
    public function getDealerAcc($q, $select) {


        $sql = "select td.delar_name,td.delar_account_no,td.delar_id,tso.sales_officer_name from tbl_dealer td
inner JOIN tbl_sales_officer tso on tso.sales_officer_id = td.sales_officer_id
where delar_account_no like '$q%' GROUP by tso.sales_officer_name";
        #echo $sql= "select * from tbl_item where status='1' and item_part_no like '$q%'";
        $query = $this->db->query($sql);
        $result = $query->result('array');
        $json_array = array();
    ...
    public function getDealerName($q, $select) {
        ...
        $sql = "select td.delar_name,td.delar_account_no,td.delar_id,tso.sales_officer_name from tbl_dealer td
inner JOIN tbl_sales_officer tso on tso.sales_officer_id = td.sales_officer_id
where td.delar_name like '$q%' GROUP by tso.sales_officer_name";
        #echo $sql= "select * from tbl_item where status='1' and item_part_no like '$q%'";
        $query = $this->db->query($sql);
        $result = $query->result('array');
        $json_array = array();
    ...
    public function getinvoAll($q, $select) {


        $sql = "SELECT `purchase_order_number`,`invoice` FROM `tbl_dealer_purchase_order` WHERE `invoice` like '$q%'  and `complete` = '1'";
        #echo $sql= "select * from tbl_item where status='1' and item_part_no like '$q%'";
        $query = $this->db->query($sql);
        $result = $query->result('array');
        $json_array = array();
...
//        
//       $sql = "select delar_account_no from tbl_dealer where delar_account_no like '$q%'";
//       #echo $sql= "select * from tbl_item where status='1' and item_part_no like '$q%'";
//        $query = $this->db->query($sql);
//        $result = $query->result('array');
//        $json_array = array();
...
//    }
    //------------------------new --------------------------------------------------------------------
    public function viewCompetitorParts(){
       ...
       // $cur_date1->modify("first day of previous month");
        ...
        date_sub($date_create,date_interval_create_from_date_string("12 months"));
        $first_date_after_12months = date_format($date_create, "Y-m-d");
        
$sql = "select
ifnull(td. delar_name,'-') as delar_name,
ifnull(td.delar_account_no,'-') as delar_account_no, 
ifnull(tso.sales_officer_name,'-') as sales_officer_name,
ifnull((SELECT DATEDIFF(CURDATE(),(tcp.added_date))),'-')  as dateDiff ,
ifnull(ti. item_part_no,'-') as item_part_no,
ifnull(ti.description,'-') as description,
ifnull(ti.selling_price,'-') as selling_price,
round(ifnull(((select sum(qty) from tbl_all_sales WHERE part_no = ti.item_part_no and acc_no = td.delar_account_no and date_edit between '".$first_date_after_12months."'  and  CURDATE() )/12),0),2) as qtytot,
ifnull(tcpd.non_tgp_part_no,'-') as non_tgp_part_no,
ifnull(tcpd.brand,'-') as brand,
ifnull(tcpd.non_tgp_importer,'-') as non_tgp_importer, 
ifnull(tcpd.non_tgp_cost_price,'-') as non_tgp_cost_price,
ifnull(tcpd.non_tgp_selling_price,'-') as non_tgp_selling_price,
ifnull(tcpd.non_tgp_current_movement,'-') as non_tgp_current_movement,
ifnull(tcpd.non_tgp_overall_movement,'-') as non_tgp_overall_movement,
(round(ifnull((select sum(qty) from tbl_all_sales WHERE part_no = ti.item_part_no and acc_no = td.delar_account_no),0) /
(ifnull((select sum(qty) from tbl_all_sales WHERE part_no = ti.item_part_no and acc_no = td.delar_account_no),0) + tcpd.non_tgp_current_movement),2 ))as xql,
round(ifnull(((ifnull((select sum(qty) from tbl_all_sales WHERE part_no = ti.item_part_no and acc_no = td.delar_account_no),0))/tcpd.non_tgp_overall_movement),'-'),2) as xax,
round(ifnull((SELECT sum(qty) from tbl_all_sales where area_id = tas.area_id and part_no = ti.item_part_no),'-'),2) as areatot,
ifnull(tcpd.part_image,'/images/comp_partImage/unload.png') as part_image
from
tbl_item ti
inner join tbl_competitor_part_detail tcpd on  tcpd.item_id = ti.item_id
inner join tbl_competitor_parts tcp on tcpd.competitor_part_id = tcp.competitor_parts_id
inner join tbl_dealer td on tcp.outlet_id = td.delar_id
inner JOIN tbl_sales_officer tso on td.sales_officer_id = tso.sales_officer_id
inner join tbl_all_sales tas on tas.acc_no = td.delar_account_no
group by ti.item_part_no order by tcp.added_date desc ";
        return $this->db->mod_select($sql);
    }

    public function getCompPartSearch1($id, $start, $end, $part, $aso){
...
ifnull(ti.description,'-') as description,
ifnull(ti.selling_price,'-') as selling_price,
round(ifnull((select sum(qty) from tbl_all_sales WHERE part_no = ti.item_part_no and acc_no = td.delar_account_no),0),2) as qtytot,
ifnull(tcpd.non_tgp_part_no,'-') as non_tgp_part_no,
ifnull(tcpd.brand,'-') as brand,
ifnull(tcpd.non_tgp_importer,'-') as non_tgp_importer, 
ifnull(tcpd.non_tgp_cost_price,'-') as non_tgp_cost_price,
...
(ifnull((select sum(qty) from tbl_all_sales WHERE part_no = ti.item_part_no and acc_no = td.delar_account_no),0) + tcpd.non_tgp_current_movement),2 ))as xql,
round(ifnull(((ifnull((select sum(qty) from tbl_all_sales WHERE part_no = ti.item_part_no and acc_no = td.delar_account_no),0))/tcpd.non_tgp_overall_movement),'-'),2) as xax,
ifnull((SELECT sum(qty) from tbl_all_sales where area_id = tas.area_id and part_no = ti.item_part_no),'-') as areatot,
ifnull(tcpd.part_image,'/images/comp_partImage/unload.png') as part_image
from
tbl_item ti
inner join tbl_competitor_part_detail tcpd on  tcpd.item_id = ti.item_id
...
inner JOIN tbl_sales_officer tso on td.sales_officer_id = tso.sales_officer_id
inner join tbl_all_sales tas on tas.acc_no = td.delar_account_no
where td.delar_id = '" . $id . "' group by td.delar_name  ";
...
ifnull(ti.description,'-') as description,
ifnull(ti.selling_price,'-') as selling_price,
ifnull((select sum(qty) from tbl_all_sales WHERE part_no = ti.item_part_no and acc_no = td.delar_account_no),0) as qtytot,
ifnull(tcpd.non_tgp_part_no,'-') as non_tgp_part_no,
ifnull(tcpd.brand,'-') as brand,
ifnull(tcpd.non_tgp_importer,'-') as non_tgp_importer, 
ifnull(tcpd.non_tgp_cost_price,'-') as non_tgp_cost_price,
...
inner join tbl_dealer td on tcp.outlet_id = td.delar_id
inner JOIN tbl_sales_officer tso on td.sales_officer_id = tso.sales_officer_id
inner join tbl_all_sales tas on tas.acc_no = td.delar_account_no where tso.sales_officer_name like '%" . $aso . "%'  group by td.delar_name ";
...
inner join tbl_dealer td on tcp.outlet_id = td.delar_id
inner JOIN tbl_sales_officer tso on td.sales_officer_id = tso.sales_officer_id
inner join tbl_all_sales tas on tas.acc_no = td.delar_account_no  where tcp.added_date between '" . $start . "' and '" . $end . "' group by td.delar_name ";
...
//delalerWlossSales
    ...
    public function delalerWlossSales() {
//
// $sql="SELECT  distinct
// ifnull(ti.item_part_no,'-') as item_part_no,
// ifnull(ti.description,'-') as description,
// ifnull(tdpod.item_qty,'-') as item_qty,
// ifnull(als.qty,'-') as qty,
// @qt:= tdpod.item_qty - als.qty as loss,
// round(ti.selling_price * (@qt),2) as lossS,
// round(count(ti.selling_price * (@qt)),2) as tot
//from  tbl_item ti
//inner join tbl_dealer_purchase_order_detail tdpod on tdpod.item_id = ti.item_id
//inner join tbl_dealer_purchase_order tdpo on tdpo.dealer_purchase_order_id = tdpod.purchase_order_id
//inner join tbl_all_sales als on als.invoice = tdpo.invoice
//where tdpo.invoice = '111111' GROUP by ti.item_part_no
// 
// 
// 
// ";
//         return $this->db->mod_select($sql);
    ...
    //dealerfromofsr
    public function getponumbers($sale) {

        $sql = "select tdpo.purchase_order_number from tbl_dealer_purchase_order tdpo

inner join tbl_sales_officer tso on tdpo.sales_officer_id = tso.sales_officer_id 
where tso.sales_officer_name  = '$sale' and tdpo.complete='1' order by tdpo.purchase_order_number";


        return $this->db->mod_select($sql);
    }
    public function getinvoponumbers($sale) {

        $sql = "select distinct(tdpo.invoice) from tbl_dealer_purchase_order tdpo
...
where tso.sales_officer_name  = '$sale' and tdpo.complete='1' order by tdpo.purchase_order_number";
    ...
    public function getalldelr($sale) {
             $app = '';
        if($sale == 'x'){
         $app ="";   
        }  else {
            
        $app ="where tso.sales_officer_name  = '$sale'"; 
            
        }
        $sql = "select td.delar_id,td.delar_name from tbl_dealer td

inner join tbl_sales_officer tso on td.sales_officer_id = tso.sales_officer_id 
{$app} ORDER BY td.delar_name DESC";


        return $this->db->mod_select($sql);
    }
    public function getalldelrAcc($sale) {
        $app = '';
        if($sale == 'x'){
         $app ="";   
        }  else {
        ...
        $sql = "select td.delar_id,td.delar_account_no,td.delar_name from tbl_dealer td

inner join tbl_sales_officer tso on td.sales_officer_id = tso.sales_officer_id {$app}
 ORDER BY td.delar_name DESC";
    ...
    public function getdatefromvalue($valufrom) {

//        $sql = "SELECT date from tbl_dealer_purchase_order  where purchase_order_number = '$valufrom' limit 1";
$sql="SELECT 
      ifnull(tdpo.date,'-') as date,
      ifnull(tdpo.invoice,'-') as invoice, 
      ifnull(tdpo.wip,'-') as wip,
      ifnull(tas.date_edit,'-') as date_edit,
      ifnull(tas.time,'-') as time
      from tbl_dealer_purchase_order tdpo
    
inner join tbl_all_sales tas on tdpo.invoice = tas.invoice 
where purchase_order_number = '".$valufrom."' limit 1" ; 


        return $this->db->mod_select($sql);
    }
    public function getpurch($valufrom) {

//        $sql = "SELECT date from tbl_dealer_purchase_order  where purchase_order_number = '$valufrom' limit 1";
$sql="SELECT 
      ifnull(tdpo.date,'-') as date,
      ifnull(tdpo.purchase_order_number,'-') as po, 
      ifnull(tdpo.wip,'-') as wip,
      ifnull(tas.date_edit,'-') as date_edit,
      ifnull(tas.time,'-') as time
      from tbl_dealer_purchase_order tdpo
...
inner join tbl_all_sales tas on tdpo.invoice = tas.invoice 
where tdpo.invoice = '".$valufrom."' limit 1" ; 


        return $this->db->mod_select($sql);
    }

    public function getDealrWLoss1($asoo, $porder,$invo) {
...
inner join tbl_dealer_purchase_order tdpo on tdpo.dealer_purchase_order_id = tdpod.purchase_order_id
inner join tbl_all_sales als on als.invoice = tdpo.invoice
inner join tbl_sales_officer tso on tso.sales_officer_id = tdpo.sales_officer_id
where sales_officer_name= '" . $asoo . "' GROUP by ti.item_part_no";
        } else if (isset($porder)) {
            $sql = "SELECT 
ifnull(ti.item_part_no,'-') as item_part_no,
ifnull(ti.description,'-') as description,
ifnull(tdpod.item_qty,'-') as item_qty,
...
inner join tbl_all_sales als on als.invoice = tdpo.invoice
inner join tbl_sales_officer tso on tso.sales_officer_id = tdpo.sales_officer_id
where tdpo.purchase_order_number = '" . $dealerId . "'   GROUP by ti.item_part_no";
        }
        return $this->db->mod_select($sql);
    }
    
    public function getDealrWLossPur($asoo, $porder,$invo){
      
       if(isset($invo) && $invo != NULL && $invo != '0'){
           
       $sql="SELECT 
 ifnull(ti.item_part_no,'-') as item_part_no,
 ifnull(ti.description,'-') as description,
 ifnull(tdpod.item_qty,'-') as item_qty,
 ifnull(ti.selling_price,'-') as sellP,

 @qt:= tdpod.item_qty as loss,
 round(ti.selling_price * (@qt),2) as lossS
 ...
 where tdpo.invoice = '".$invo."'";
    ...
    public function getDealrWLossinvo($asoo, $porder,$invo){
         if($invo == '' ||  $invo == '0'){
         return 1;  
       }else{
    $sql="SELECT part_no,qty FROM tbl_all_sales WHERE invoice = '".$invo."'";
        return $this->db->mod_select($sql);
       }
    }


    public function delalerWlossSalesfromPur() {
        
  
        
        
    }
    public function delalerWlossSalesfromAllS() {
       
        
    }
    public function delaC($val) {
       $sql="select delar_id,delar_account_no "
               . "from tbl_dealer where delar_id = '".$val."'";
       
        return $this->db->mod_select($sql);
        
    }
    public function delrN($val) {
       $sql="select delar_id,delar_name "
               . "from tbl_dealer where delar_id = '".$val."'";
      ...
      public function getDelponumbers($sale) {

        $sql = "SELECT `purchase_order_number` FROM `tbl_dealer_purchase_order` WHERE `dealer_id` = '".$sale."' and complete='1' order by purchase_order_number";
