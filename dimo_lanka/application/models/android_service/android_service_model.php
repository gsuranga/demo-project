...
//        echo $data['deliver_order_id'];
        ...
        $tbl_bank_deposit_payment = array(
            ...
            'account_no' =>$json_decode->account_no,
            'sales_officer_id' => $data['sales_rep_id'],
            'tour_status' => 0,
        ...
        $this->db->insertData("tbl_tab_bank_deposit_payment",$tbl_bank_deposit_payment);
//        $this->db->insertData("tbl_bank_deposit_payment", $tbl_bank_deposit_payment);
    }
    
    //insert_return_from_tab-----------------------------
    
    public function insert_return_from_tab($data){
        
        $json_decode = json_decode($data['json_string']);

        $full_path = '';
        if ($json_decode->image != '') {
            $image_name = 'return'. date('Y-m-d H-i-s');
            //   $image_name="x";
            $full_path = "./images/returnimg/{$image_name}.jpg";
            file_put_contents($full_path, base64_decode($json_decode->image));
            chmod($full_path, 0777);
        }
//        echo $data['deliver_order_id'];
        ...
        $tbl_bank_deposit_payment = array(
            ...
            'part_no' => $json_decode->part_no,
            'qty' => $json_decode->qty,
            'amount' => $json_decode->value,
            'remarks' => $json_decode->remarks,
            'images' => $full_path,
            'reason' => $json_decode->reason,
            'add_date' => $json_decode->date,
            'add_time' => $json_decode->time,
            'sales_officer_id' => $data['sales_rep_id'],
            'deliver_order_id' => $json_decode->deliver_order_id);
            


        $this->db->insertData("tbl_return_tab",$tbl_bank_deposit_payment);
      }
...
//        echo $data['deliver_order_id'];
      ...
      $full_path = '';
        if ($json_decode->image != ''){
            $image_name = 'cheque'.date('Y-m-d H-i-s');
            //   $image_name="x";
            $full_path = "./images/chequeImg/{$image_name}.jpg";
            file_put_contents($full_path, base64_decode($json_decode->image));
            chmod($full_path, 0777);
        }
...
//    insert_target_collectDay
    
    
    public function insert_target_collectDay($data){
        //print_r($data);
         $json_decode = json_decode($data['json_string']);  
          ...
          $where = array(
            ...
            $date = array(
                'tgt_collection_dt' => $json_decode->tgt_collection_dt
            );
            $this->db->__beginTransaction();
            $this->db->update('tbl_dealer_deliver_order', $date, $where);
...
//        echo $data['deliver_order_id'];
        ...
        if ($json_decode->path != '') {
            $image_name = $data['sales_rep_id'].date('Y-m-d H-i-s');
            $full_path = "./images/vouture/{$image_name}.jpg";
            file_put_contents($full_path, base64_decode($json_decode->path));
            chmod($full_path, 0777);
        }
        ...
        $tbl_cash_payment = array(
            ...
            'img_path' => $full_path,
            'remarks' => $json_decode->remarks,
            'voucher_no' => $json_decode->voucher_no,
            'status' => 0,
        );
        $this->db->insertData("tbl_garage_loyality_vouture", $tbl_cash_payment);
...
";
    ...
    public function insert_sales_order() {
        ...
        foreach ($data->order as $order) {
            //count vat
           // $order_without_vat = $order->BillAmount + (($order->BillAmount * $vat[0]->amount) / 100);
            $order_without_vat =($order->BillAmount*100)/($vat[0]->amount+100);
            $purchase_oder_no="";
        ...
        $dealerDetails = $this->delar_model->getDealerDetails("delar_id", $order->dealer_id);
        ...
        $purchase_oder_no = $this->auto_generate_id->generateID($lastPurchaseOrderID, $reference_str);
            ...
            if(isset($order->remark_1))
            {
                $remark_1s=$order->remark_1;
            }
             if(isset($order->remark_2))
            {
                $remark_2s=$order->remark_2;
            }
            ...
            $tbl_dealer_purchase_order = array(
                ...
                'remarks1' => $remark_1s,
                'remarks2' => $remark_2s,
                'purchase_order_number'=>$purchase_oder_no,
                'status' => 1,
  /*new code..*/'order_type' => $order_type
            );
...
AND tv.time_stamp > " . $time_stamp;
    ...
    public function getFastMoovingItemsInArea($dealer_id) {
       ...
       $sql="select 
              ...
              sum(tas.qty)/6 as quantity
             
           from
             tbl_item ti
             inner join
             tbl_all_sales tas ON ti.item_part_no = tas.part_no
           where
             tas.date_edit between '".$first_date_after_6months."' and '" .$last_date_of_previous_month. "'
             and tas.area_id = (select 
               area_id
                    from
                  tbl_all_sales
    where
    acc_no = '".$dealer_id."'
     ...
     and tas.acc_no != '".$dealer_id."'
  group by ti.item_id  ORDER BY qty DESC LIMIT 6";
...
//order by quantity desc limit 10  ";
    ...
    public function getNotAchivedTargets($officer_id,$target_month,$dealer_id) {
        
        
        $datex=explode("-",$target_month);
        $year =  $datex[0];
        $month = $datex[1];
       //-------------------------------new sql-4:23 PM 9/23/2015----------------------------------------
      $sql=" select  
        ti.item_id, 
        ti.item_part_no,
        ti.description,
@tot:= ifnull((SELECT sum(item_qty) FROM tbl_dealer_purchase_order tdpo 
inner join tbl_dealer_purchase_order_detail tdpd on 
tdpo.purchase_order_id = tdpd.purchase_order_id where 
date LIKE '".$target_month."-%' and complete=1
and sales_officer_id =" . $officer_id . " and dealer_id =".$dealer_id." and item_id = ti.item_id),0) as tot, 

@min :=ifnull( (select sum(minimum_qty) from tbl_monthly_target_detail tmtd
inner join tbl_sales_officer_monthly_target tsom on tmtd.monthly_target_id = tsom.monthly_target_id
where sales_officer_id = " . $officer_id . " and year =".$year." and month = ".$month." and "
. "dealer_id = ".$dealer_id." and item_id = ti.item_id),0) as min,

@min - @tot as BBF,
ifnull((SELECT sum(item_qty) FROM tbl_dealer_purchase_order tdpo 
inner join tbl_dealer_purchase_order_detail tdpd on 
tdpo.purchase_order_id = tdpd.purchase_order_id where 
date LIKE '".$target_month."-%' and complete=1
and sales_officer_id = " . $officer_id . " and dealer_id =".$dealer_id." and item_id = ti.item_id),0) as total_actual,


(select sum(minimum_qty) from tbl_monthly_target_detail tmtd
inner join tbl_sales_officer_monthly_target tsom on tmtd.monthly_target_id = tsom.monthly_target_id
where sales_officer_id = " . $officer_id . " and year =".$year."  and month = ".$month." and dealer_id = ".$dealer_id." and item_id = ti.item_id )as tot_minim_qty,

if(tmtd.minimum_qty - ifnull((SELECT sum(item_qty) FROM tbl_dealer_purchase_order tdpo 
inner join tbl_dealer_purchase_order_detail tdpd on 
tdpo.purchase_order_id = tdpd.purchase_order_id where date LIKE '".$target_month."-%' and complete=1
and sales_officer_id = " . $officer_id . " and dealer_id =".$dealer_id." and item_id = ti.item_id),0)<=0,
tmtd.minimum_qty=0,tmtd.minimum_qty -ifnull((SELECT sum(item_qty) FROM tbl_dealer_purchase_order tdpo 

inner join tbl_dealer_purchase_order_detail tdpd on
tdpo.purchase_order_id = tdpd.purchase_order_id where date LIKE '".$target_month."-%' and complete=1
and sales_officer_id = " . $officer_id . " and dealer_id =".$dealer_id." and item_id = ti.item_id),0)) as tot_minimum_qty,
...
if( tmtd.additional_qty + tmtd.minimum_qty -ifnull((SELECT sum(item_qty) FROM tbl_dealer_purchase_order tdpo 
inner join tbl_dealer_purchase_order_detail tdpd on 
tdpo.purchase_order_id = tdpd.purchase_order_id where date LIKE '".$target_month."-%' and complete=1
and sales_officer_id = " . $officer_id . " and dealer_id =".$dealer_id." and item_id = ti.item_id),0)<=0,

tmtd.additional_qty=0,tmtd.additional_qty + tmtd.minimum_qty - 
ifnull((SELECT sum(item_qty) FROM tbl_dealer_purchase_order tdpo 
inner join tbl_dealer_purchase_order_detail tdpd on 

tdpo.purchase_order_id = tdpd.purchase_order_id where date LIKE '".$target_month."-%' and complete=1
and sales_officer_id =" . $officer_id . " and dealer_id = ".$dealer_id." and item_id = ti.item_id),0))
,tmtd.additional_qty) as tot_additional_qty

from tbl_item ti inner join tbl_monthly_target_detail tmtd on tmtd.item_id = ti.item_id 
inner join tbl_sales_officer_monthly_target tmt on tmtd.monthly_target_id =tmt.monthly_target_id
where sales_officer_id = " . $officer_id . " and year =".$year."  and month = ".$month." and dealer_id = ".$dealer_id." group by ti.item_id " ;
 
        
        //----------------------------------------------------------------------------------
        
        
//  echo  $sql ="SELECT  Distinct ti.item_id, 
//ti.item_part_no,
//ti.description,
//tmtd.minimum_qty as a,
//tmtd.additional_qty as b ,
//ifnull((SELECT sum(item_qty) FROM tbl_dealer_purchase_order tdpo 
//inner join tbl_dealer_purchase_order_detail tdpd on 
//tdpo.purchase_order_id = tdpd.purchase_order_id where date LIKE '".$target_month."-%' and complete=1
//and sales_officer_id = " . $officer_id . " and dealer_id =" . $dealer_id . " and item_id = ti.item_id),0) as total_actual,
...
//inner join tbl_dealer_purchase_order_detail tdpd on 
//tdpo.purchase_order_id = tdpd.purchase_order_id where date LIKE '".$target_month."-%' and complete=1
//and sales_officer_id = " . $officer_id . " and dealer_id =" . $dealer_id . " and item_id = ti.item_id),0)<=0,tmtd.minimum_qty=0,tmtd.minimum_qty -ifnull((SELECT sum(item_qty) FROM tbl_dealer_purchase_order tdpo 
//inner join tbl_dealer_purchase_order_detail tdpd on 
//tdpo.purchase_order_id = tdpd.purchase_order_id where date LIKE '".$target_month."-%' and complete=1
//and sales_officer_id = " . $officer_id . " and dealer_id =" . $dealer_id . " and item_id = ti.item_id),0)) as tot_minimum_qty,
//
...
//inner join tbl_dealer_purchase_order_detail tdpd on 
//tdpo.purchase_order_id = tdpd.purchase_order_id where date LIKE '".$target_month."-%' and complete=1
//and sales_officer_id = " . $officer_id . " and dealer_id =" . $dealer_id . " and item_id = ti.item_id),0)<=0,tmtd.additional_qty=0,tmtd.additional_qty+tmtd.minimum_qty -ifnull((SELECT sum(item_qty) FROM tbl_dealer_purchase_order tdpo 
//inner join tbl_dealer_purchase_order_detail tdpd on 
//tdpo.purchase_order_id = tdpd.purchase_order_id where date LIKE '".$target_month."-%' and complete=1
//and sales_officer_id = " . $officer_id . " and dealer_id =" . $dealer_id . " and item_id = ti.item_id),0))
//
...
//
//
//FROM tbl_sales_officer_monthly_target tsomt 
//inner join 
//tbl_monthly_target_detail tmtd on tmtd.monthly_target_id = tsomt.monthly_target_id
//inner join 
//tbl_item ti on ti.item_id = tmtd.item_id
//where sales_officer_id = " . $officer_id . " and year =" . $year . "  and month = " . $month . " and dealer_id = " . $dealer_id . "";

//        $sql = "SELECT ti.item_id, 
//ti.item_part_no,
//ti.description,
//tmtd.minimum_qty as tot_minimum_qty,
//tmtd.additional_qty as tot_additional_qty ,
//0 as total_target,
//0 as variance,
//0 as total_actual
...
//   or actual_qty < total_target";
    ...
    public function getNotAchivedTargets1($dealer_id, $target_month) {
...
//======================================widuranga jayawickram 2104-12-30===================================
    public function MonthlyTarget($data) {
        ...
        $tbl_sales_officer_monthly_target = array(
            ...
            'year' => $json_decode->target_json->year,
            'month' => $json_decode->target_json->month,
...
//----------------------------------------nnnnnnnnnnnnnnnnnnnnnnnnnnnnnn
    public function comperiterPart($data) {
        $this->save_json_to_txt(json_encode($_REQUEST));
...
//                'non_tgp_movement_area' => $value->ety,
    ...
    function update_dealer_location($data){
        ...
        try {
            ...
            if(isset($json_decode->data->dealer_password)){
                $password = array(
               'password' => $this->encrypt->encode($json_decode->data->dealer_password)
                        
                );
            $this->db->where('employee_id', $dealer_id);
            $this->db->update('tbl_user', $password); 
...
//    update_dealer_User_name_password
    public function update_dealer_Username_password($data){
         $json_decode = json_decode($data['json_string']);
         $this->load->library('encrypt');
         
         $dealer_id = $_REQUEST['dealer_id'];
         //encrypt->encode($json_decode->userName)
            $date = array(
               
                'password' => $this->encrypt->encode($json_decode->password)
            ...
            $this->db->__beginTransaction();
            $this->db->update('tbl_user', $date, $where);
    ...
    public function avg_movement_item_dealer($dealer,$item_id) {
        
            $sql="select ROUND(ifnull(sum(dpd.item_qty)/6,0),2) as avg_movement from tbl_dealer_purchase_order dp inner join tbl_dealer_purchase_order_detail dpd on dpd.purchase_order_id=dp.purchase_order_id 
            where dp.complete=1 and dpd.item_id = '$item_id' and dp.date between date_sub(curdate(), interval 6 month) and curdate()and dp.dealer_id='$dealer'";
  
       return $this->db->mod_select($sql);
      
    } 

    public function save_json_to_txt($param) {
        
        $myfile = fopen("./images/comp_partImage/newfile.txt", "w+") or die("Unable to open file!");
