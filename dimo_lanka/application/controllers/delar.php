...
//
        ...
        $this->form_validation->set_rules('delar_num', 'Dealer_Number', 'required');
    ...
    //new edit for update delar--------------------------
//    public function drawViewAllDelarforsalesoficerfroup() {
//        $this->load->model('delar/delar_model');
//        $viewAllDealer = $this->delar_model->viewAllDealerforsalesoficer();
//        $this->template->draw('delar/addmoredetailsdealer', false, $viewAllDealer);
//    }
    ...
    //new delar update from tab----------------------------
    
    public function drawIndexupdatemoredetailsnew() {
        $this->template->attach($this->resours);
        $this->template->draw('delar/indexviewmorenew', true);
    }
    ...
    //new for tab delar up---------------------------
    public function drawDealertabupdate() {

        $delar_id = $_REQUEST['tokendealer_id'];
        $this->load->model('delar/delar_model');
      
        $dataset = $this->delar_model->viewAllDealerinfor($delar_id);
        $this->template->draw('delar/addmoredetailsdelarfromTab', false, $dataset);
    }
...
//9:36 AM 10/16/2015
    ...
    public function createDelarNew() {
      ...
      $this->form_validation->set_rules('delar_name', 'Dealer Name');
        $this->form_validation->set_rules('mobilep', 'Phone Number');
       $this->form_validation->set_rules('telp', 'Phone Number');
        $this->form_validation->set_rules('delar_shop_name', 'Shop Name');
       $this->form_validation->set_rules('contact_land', 'Phone Number');
        $this->form_validation->set_rules('contact_mobile', 'Phone Number');
