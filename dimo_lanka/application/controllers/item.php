...
class item extends C_Controller {
    ...
    public function upload_now(){
          ...
          mkdir('./upload_vsd/', 0777, TRUE);
            ...
            $image_path = realpath(APPPATH .'../upload_vsd');
...
//            $config['max_height'] = '768';
            ...
            if (!$this->upload->do_upload('fileName')) {
               ...
               // $this->load->view('item/upload_items', $error);
            } else {
               $data = array('upload_data' => $this->upload->data());
//                $this->load->view('item/upload_items', $data);
               $this->load->model('item/item_model');
               //print_r($data);
               $sql= $this->item_model->save_new_csv($data);
               print_r($sql);
                if ($sql == 1) {
               $this->load->model('item/item_model');
               $sql1= $this->item_model->insert_cvs();
//               print_r($sql1);
                $this->template->draw('item/view_all_part_numbers', false, $sql1);    
...
//            $config['max_height'] = '768';
           ...
           $min= $this->load->library('upload', $config);
...
//                $this->load->view('item/upload_items', $data);
               $this->load->model('item/item_model');
               //print_r($data);
               $sql= $this->item_model->save_csv($data);
               print_r($sql);
...
//               print_r($sql1);
                $this->template->draw('item/view_all_part_numbers', false, $sql1);    
                 $this->session->set_flashdata('upload_vsd', '<br><span style="font-size: 13px;background-color: #FFFFFF;color:green;border:solid 1px #00BFFF;padding:2px;border-radius: 5px 5px 5px 5px">Download</spam>');
                redirect('item/drawIndexUploadItems');
                }    
            else {
                $this->session->set_flashdata('upload_vsd', '<br><span style="font-size: 13px;background-color: #FFFFFF;color:red;border:solid 1px #00BFFF;padding:2px;border-radius: 5px 5px 5px 5px">Download Failed</spam>');
                redirect('item/drawIndexUploadItems');
            } 
