...
class tour_iteneray extends C_Controller {
    ...
    public function drawDistricLoad() {
        ...
        public function drawCity() {
        $District_id=$_REQUEST['id'];
        $this->load->model('tour_iteneray/tour_iteneray_model');
        $viewAll = $this->tour_iteneray_model->viewAllCity($District_id);
        $this->template->draw('tour_iteneray/load_city', FALSE, $viewAll);
    }
...
//        //dealer_type
//        
//        $q = strtolower($_GET['type']);
//        $this->load->model('tour_iteneray/tour_iteneray_model');
//        $column = array('ssss');
//        $result = $this->tour_iteneray_model->get_dealer_type($q, $column);
//        $result_data = array('label' => 'no result...');
//        if (count($result) > 0) {
//            echo json_encode($result);
//        } else {
...
//    }
  ...
  //----------------------------------------------------------------------------------------------------  
    ...
    public function get_dealerName() {
        ...
        $q = strtolower($_GET['term']);
        $this->load->model('tour_iteneray/tour_iteneray_model');
        $column = array('delar_name', 'delar_id','delar_account_no', 'city_name','city_id');
        $result = $this->tour_iteneray_model->get_DealerNames($q, $column, $Town_id);
        $result_data = array('label' => 'no result...');
        if (count($result) > 0) {
            echo json_encode($result);
        } else {
        ...
        public function dealer_Acco() {
        $Acc = $this->input->get('acc_no');
        $q = strtolower($_GET['term']);
        $this->load->model('tour_iteneray/tour_iteneray_model');
        $column = array( 'delar_account_no','delar_id','delar_name', 'city_name','city_id');
        $result = $this->tour_iteneray_model->get_DealerAcco($q, $column, $Acc );
        $result_data = array('label' => 'no result...');
        if (count($result) > 0) {
            echo json_encode($result);
        } else {
