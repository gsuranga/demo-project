...
//        }
    ...
    public function insert_target_collectDay(){
       //web service to take target Coll Date
     
        $result = $this->android_service_model->insert_target_collectDay($_REQUEST);
        echo json_encode(array('result' => 'true'));
    }
...
//==============widuranga jayawickrama==========end============================================================
    ...
    public function getNotAchivedTargets1() {
        //$officer_id = $_REQUEST['officer_id'];
        $notAchivedTargets = $this->android_service_model->getNotAchivedTargets1($_REQUEST['dealer_id'],$_REQUEST['target_month']);
     ...
     public function getNotAchivedTargets() {
        ...
        $dealer_id = $_REQUEST['dealer_id'];
        $notAchivedTargets = $this->android_service_model->getNotAchivedTargets($officer_id,$target_month,$dealer_id);
        $jsonS = array("not_achieved" => $notAchivedTargets);
        $json_encode = json_encode($jsonS);
        print_r($json_encode);
        return $json_encode;
...
//        $this->save_json_file_x('loc');
    ...
    //new andro_sevice for update dealer 
    
    public function update_dealer_User_name_password() {
//        $this->save_json_file_x('loc');
        $result = $this->android_service_model->update_dealer_Username_password($_REQUEST);
        $test = array('result' => $result);
        echo json_encode($test);
    }
...
//    }
    ...
    //-----------------new ------------------------------------------------------------------------------------------------------
    public function insert_return_from_tab() {
        // echo "gdgfgh";
       
        $result = $this->android_service_model->insert_return_from_tab($_REQUEST);
        $test = array('result' => "true");
        echo json_encode($test);
    }
...
//        
    ...
    public function updateservice() {
	 ...
	 public function avg_movement_item_dealer() {
        $dealer_id = $_REQUEST['dealer_id'];
        $item_id = $_REQUEST['item_id'];

        $this->load->model('android_service/android_service_model');
        $result = $this->android_service_model->avg_movement_item_dealer($dealer_id, $item_id);
        
        //print_r($result);
         $sss = array('avg_movement' => $result[0]->avg_movement);
         echo json_encode($sss);
    }
