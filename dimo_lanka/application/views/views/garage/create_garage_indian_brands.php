<?php

/*
 * To change this template, choose Tools | Templates
 * and open the template in the editor.
 */
?>
<?php
$garage_name = array(
    'id' => 'garage_name',
    'name' => 'garage_name',
    'type' => 'text',
    'onfocus' => 'get_all_garage_name();'
);
$garage_id = array(
    'id' => 'garage_id',
    'name' => 'garage_id',
    'type' => 'hidden'
);
$create_garage_indian = array(
    'id' => 'create_garage_indian',
    'name' => 'create_garage_indian',
    'type' => 'submit',
    'value' => 'Create Garage Indian Brand'
);

?>
<?php echo form_open('garage/inserindianDelar'); ?>
<table width="80%" align="center">
    <tr>
        <td>Garage Name</td>
        <td>
            <?php echo form_input($garage_name);?>
            <?php echo form_input($garage_id);?>
        </td>
    </tr>
    <tr>
        <td>Indian Brand</td>
        <td>
            <table width="80%" id="indian_brand">
                <tr>
                    <td>
                        <img style="width: 20px; height: 20px" type="button"   onclick="add_new_indian_brand();" src="http://localhost/dimo_lanka/public/images/add2.png">

                    </td>
                    <td>
                        <input type="text" id="txt_indian_brand_1" name="txt_indian_brand_1" placeholder="Select Vehicle Part Type" autocomplete="off" onfocus="get_all_vehicle_part_type_indian('1');"/>
                        <input type="hidden" id="txt_indian_brand_id_1" name="txt_indian_brand_id_1"/>
                    </td>
                    <td>
                        <img style="width: 20px; height: 20px" type="button"   onclick="remove_tgp_part_type();" src="http://localhost/dimo_lanka/public/images/delete2.png">

                    </td>
                </tr>
            </table>
        </td>
    </tr>
    <tr>
        <td></td>
        <td>
            <table align="right">
                <tr>
                    <td><?php echo form_input($create_garage_indian); ?></td>
                </tr>
            </table>
        </td>
    </tr>
    <input type="hidden" name="tokn_row" id="tokn_row" value="1">
</table>
<?php echo form_close(); ?>