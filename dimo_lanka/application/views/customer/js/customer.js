...
function view_drive_details_admin(details) {
    ...
    $j.ajax({
        ...
        success: function (data) {
            ...
            var hlml = '<table width="100%"  style="color: #333; font-weight: bolder;  font-size: 20px;">'
                    + '<tr style="background: #4297C0;">'
                    + '<td  colspan="2" align="center">Drive Details</td></tr></table>'
...
function Other_vehicles(details) {
    ...
    $j.ajax({
        ...
        success: function (data) {
            ...
            var hlml = '<table width="100%"  style="color: #333; font-weight: bolder;  font-size: 20px;">'
                    + '<tr style="background: #4297C0;">'
                    + '<td  colspan="2" align="center">Customer Other Vehicles</td></tr><br>'
                    + '</table>'
                    + '<tr><td></td><td style="font-size: 30px">' + x[0].ve1 + '</td></tr><br>'
            //   + '<tr><td style="font-size: 15px;"><b><u>Drive Contact No:</u></b></td><td></td><td style="font-size: 15px">' + x[0].driverConNo + '</td></tr>'
            //       + '<tr><td style="font-size: 15px;"><b><u>Driver Email:</u></b></td><td></td><td style="font-size: 15px">' + x[0].driverEmail + '</td></tr></table>';
            for (var i = 1; i < x.length; i++) {
                hlml += '<tr><td></td><td style="font-size: 30px">' + x[i].ve1 + '</td></tr>'
//                '<tr><td></td><td></td><td style="font-size: 15px"></td></tr>'
//                '<tr><td></td><td></td><td style="font-size: 15px"></td></tr>'
...
//                    alert('sdsd');
                   // var y = JSON.parse(data);
                    ...
                    for (var i = 1; i < x.length; i++) {
                        hlml += '<tr><td>Vehicle 2:-</td><td><input type="text" value="' + x[i].ve1 + '" name="ve'+ i +'" id="ve'+ i +'"/></td></tr>'
                        +'<td><input type="hidden" value="' + i + '" name="token_arres_of_trevel" id="token_arres_of_trevel"/>'
                +'<td><input type="hidden" value="' + x[i].other_vehicle_id + '" name="cus_id'+ i +'" id="cus_id'+ i +'"/>';
                    }
                    hlml +=    '</td></tr></table><table><tr><td><input type="submit" name="btn_vehi_details" id="btn_vehi_details" value="Submit"/></td></tr></table>';
                    $j.colorbox({
                        html: '<div align="center"><h1>Update Vehicle Details</h1>' + hlml + '<div>',
                        height: "45%",
                        width: "35%",
...
////                        + '<input type="hidden" id="token_Drivers_no'+i+'" name="token_Drivers_no'+i+'" value="1"/>'
...
//            $j.colorbox({
//                html: '<div align="center"><h1>Update Drive Details</h1>' + hlml +
//                        '</div>',
//                height: "55%",
//                width: "50%",
//                opacity: 0.50
...
<td><input type='text' id='driver_name_new_" + z + "'   name='driver_name_new_" + z + "' placeholder='Enter Driver' /></td><td><input type='button' id='remove_driver' name='remove_driver_" + z + "' onclick='removeUpdate_Driver(" + z + ")' style='font-size: small' value='X'  /></td></tr>");
    z++;
}
            ...
            $j.ajax({
                ...
                success: function (data) {
                    ...
                    var hlml = '<form action="update_Contact_Details"><table>'
                            ...
                            + '<input type="hidden" value="' + details + '" name="vehi_id" id="vehi_id"/></table></form>';
//                    for (var i = 0; i < y.length; i++) {
...
function get_garage_name(id) {
    ...
    $j("#garage_name_" + id).autocomplete({
        ...
        dataType: "jsonp",
        select: function (event, data) {
            $j('#garage_id_' + id).val(data.item.garage_id);
        }
    });
}
//dealer names getting =============================== stsrt===
function get_garage1() {

    $j("#garage_name_1").autocomplete({
        source: "get_gatages",
        width: 265,
        selectFirst: true,
        minlength: 1,
        dataType: "jsonp",
        select: function (event, data) {
            $j('#garage_name_1').val(data.item.garage_name);
        }
    });
}
//======================== get  dealer name =====
...
function addParts1() {
    ...
    $j('#dealerRegForm1').append('<tr id="row_' + dr1 + '"><td width="170" align="right"><input type="button" name="pls_dealer" id="pls_dealer' + dr1 + '"  onclick="addParts1(' + dr1 + ');" style="font-size: larger" value="+"/>'
            ...
            + '</td><td>'
            + '<input type="text" id="dealer_name_' + dr1 + '" name="dealer_name_' + dr1 + '" size="10px" autocomplete="off" onclick="get_dealer123(' + dr1 + ');"  placeholder="Select Or Enter New Shop">'
            + ' <input type="hidden" id="dealer_name_id_' + dr1 + '" name="dealer_name_id_' + dr1 + '" size="10px" autocomplete="off">'
            + ' </td><td>'
            + ' <input type="text" id="garage_name_' + dr1 + '" name="garage_name_' + dr1 + '" size="10px" autocomplete="off">'
            + ' <input type="hidden" id="garage_name_id_' + dr1 + '" name="garage_name_id_' + dr1 + '" size="10px" autocomplete="off">'
...
function dealerPurchasePart() {
    ...
    $j('#dealerPurchasePart').append('<tr  align="right" id="remove_PurchasePart_' + D + '"><td ><input type="button" name="pls_driver" id="pls_driver_id_' + D + '"  onclick="dealerPurchasePart();" style="font-size: larger" value="+"/></td>' +
            '<td>'
...
//            '<td></td>' +
            ...
            '<td><input type="hidden" id="selected_name_Hid_' + D + '" name="selected_name_Hid_' + D + '"></td>' +
