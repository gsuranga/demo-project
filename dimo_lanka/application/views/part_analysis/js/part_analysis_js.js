...
//        dataType: "jsonp",
//        focus: function(event, data) {
//            $j('#part_no').val(data.item.item_part_no);
//            $j('#part_id').val(data.item.item_id);
//            $j('#dascrptn').val(data.item.description);
//            $j('#saell_price').val(data.item.selling_price);
//           
//   
//            
//          
...
function get_part_no() {
    ...
    $j("#partnumber").autocomplete({
        ...
        focus: function (event, data) {
            ...
            $j('#price').val(data.item.selling_price);
...
function get_part_number(id) {
    ...
    $j("#prt_no_" + id).autocomplete({
        source: "getPartNumber?dlr_id=" + $j('#dlr_id').val(),
        width: 265,
        selectFirst: true,
        minlength: 1,
        dataType: "jsonp",
        focus: function (event, data) {
            ...
            $j('#dimo_to_dealer_' + id).val(data.item.avg_momnt);
...
function update_part_analysis() {

    $j.ajax({
        type: 'POST',
        url: 'managepart',
        data: {
            data: $j('#frm_part_analys').serializeArray()
        },
        success: function () {
