...
function createDeliverCall(x) {
   
    window.open(URL + "dealer_accept_order/drawIndex_edit_delear_Accept_order?k=1&acccCall=" + x, null, "height=600,width=1200,status=yes,toolbar=no,menubar=no,top=20,left=20");
}
...
//addDelearCall
function addDelearCall() {
   // if (!confirm('Are you sure you want to Accept Call Order?')) {
//        ev.preventDefault();
//        return false;
//    } else {
        var dataes = $j('#tvc_form').serialize();
        ...
        $j.ajax({
            url: 'getOrderDetailForAcceptCall',
            type: 'POST',
            data: dataes,
            success: function(data) {
