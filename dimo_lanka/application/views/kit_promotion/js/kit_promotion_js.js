...
function get_part_number(id) {
    ...
    $j("#part_number_" + id).autocomplete({
        ...
        focus: function(event, data) {
            ...
            $j('#mod'+id).val(data.item.selling_price);
            get_tot();
        }
    });
}
...
//            var co = 1;
            if (y.length > 0) {
                for (var i = 0; i < y.length; i++) {
                    co =i+1;
                 addRow('adrw'+co, co); 
                 // alert( y[i]['sales_officer_id'] + y[i]['sales_officer_name']+ y[i]['branch_name']); 
                 
                    $j('#aso_name' + co).val(y[i]['sales_officer_name']);
                     $j('#branch'+co).val(y[i]['branch_name']);
                     $j('#aso_id'+co).val(y[i]['sales_officer_id']);
                    // $j('#aso_id1').val(y[i]['sales_officer_id']);
                }
...
//            + '<td><input type="button" name="adrw_' + id + '" id="adrw_' + id + '" value="+" onclick="addRow(this.id, ' + id + ');"></td>'
            ...
            + '<td><input type="text" name="quantity_' + id + '" value="0" id="quantity_' + id + '" style="text-align: right" onkeyup="create_tot_qty();"></td>'
//            + '<td><input type="button" id="deletrow_' + id + '" name="deletrow_' + id + '" value="-" onclick="deleteRow(' + id + ');"></td>
            +'</tr>'
            );
...
////    alert(id);
//      $j.ajax({
//        url: 'drawAsojs?aso=' + id,
//        method: 'GET',
//        success: function (data) {
//            alert(data);
////            $j('#aso_id' + id).val(data.sales_officer_id);
////            $j('#branch' + id).val(data.branch_name);
//            //alert(data);
////             $j('#aso_namesl' + id).val(data.sales_officer_id);   
// var x = JSON.parse(data);
//            //alert(x);
//            console.log(x);
//            var y = x[0];
//            //   console.log(y[0]['s_o_purchase_order_id']);
//            $j('#aso_names'+id).empty();
//            if (y.length > 0) {
//                for (var i = 0; i < y.length; i++) {
//                    $j('#aso_names'+id).append(
//                      
//                         '<option value="'+ y[i]['sales_officer_name'] +'"  selected  getbranch(value) >' + y[i]['sales_officer_name'] + '</option>'
//                     
//                            );
//                }
//            } else {
